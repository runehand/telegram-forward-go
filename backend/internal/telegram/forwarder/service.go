...
func NewService(cfg config.TelegramConfig, logger *zap.Logger, store messageStore) *Service {
	return &Service{cfg: cfg, log: logger, store: store}
}
...
func messageLogFields(m *tg.Message) []zap.Field {
	return []zap.Field{
		zap.Int("id", m.ID), zap.Int("date_unix", m.Date), zap.String("from", peerToString(m.FromID)), zap.String("peer", peerToString(m.PeerID)),
		zap.Bool("out", m.Out), zap.Bool("silent", m.Silent), zap.Bool("mentioned", m.Mentioned), zap.Bool("media_unread", m.MediaUnread),
		zap.Bool("post", m.Post), zap.Bool("legacy", m.Legacy), zap.Int("edit_date_unix", m.EditDate), zap.Int("forwards", m.Forwards),
		zap.Int("views", m.Views), zap.Int("replies", repliesCount(m.Replies)), zap.Bool("has_text", m.Message != ""), zap.String("text", m.Message),
		zap.String("media_type", typeName(m.Media)), zap.String("reply_to_type", typeName(m.ReplyTo)), zap.String("fwd_from_type", typeName(m.FwdFrom)),
		zap.String("reply_markup_type", typeName(m.ReplyMarkup)), zap.Int("entities_count", len(m.Entities)), zap.Any("raw_message", m),
	}
}
